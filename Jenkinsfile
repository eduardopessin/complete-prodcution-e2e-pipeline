pipeline{
    ...
    stages{
        ...
        stage("Trivy Scan") {
            steps {
                script {
		   sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy ${IMAGE_NAME}-${IMAGE_TAG}  --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
                }
            }
